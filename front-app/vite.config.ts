...
// somewhere other than the default :8800.
const BACKEND_URL =
  process.env.PLATFORM_BACKEND_URL ?? "http://127.0.0.1:8800";

export default defineConfig({
  plugins: [react()],
  server: {
    port: 8873,
    strictPort: true, // fail loudly on a taken port instead of drifting to 8874
    proxy: {
      "/api": {
        target: BACKEND_URL,
        changeOrigin: true,
