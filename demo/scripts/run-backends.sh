#!/usr/bin/env bash
# Launch the HexaUI backends for local dev — agent-server (:9080) + proxy (:9000)
# on a throwaway SQLite DB (no Postgres needed). Run from the repo root under WSL:
#
#     bash demo/scripts/run-backends.sh
#
# Then in a Windows terminal:  cd front-app && npm run dev   (→ http://localhost:9173)
set -euo pipefail
...
export PLATFORM_AGENT_BACKEND_URL="http://127.0.0.1:9080"
...
cleanup() { fuser -k 9080/tcp 9000/tcp 2>/dev/null || true; }
...
echo "starting agent-server :9080 (AGENT_ENABLE_LLM=$AGENT_ENABLE_LLM) …"
PYTHONPATH=demo/agent-server/src \
  demo/agent-server/.venv/bin/python -m agent_server &

echo "starting proxy :9000 (sqlite: $PLATFORM_DATABASE_URL) …"
PYTHONPATH=demo/proxy/src:demo/packages/hexa-events/src \
  demo/proxy/.venv/bin/python -m platform_backend &

echo "both up. open the web app with: (in front-app)  npm run dev  →  http://localhost:9173"
