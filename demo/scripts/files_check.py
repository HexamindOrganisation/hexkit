...
# every subsequent call is authenticated (the proxy's routes all require auth
# since the multi-user swap).
_token = c.post(
    "/auth/signup",
    json={"email": "files-check@example.com", "password": "hexademo"},
).json()["access_token"]
